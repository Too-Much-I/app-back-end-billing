...
public class BillingMongoIndexInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        ensureCollection(BENEFIT_COLLECTION);
        ...
        ensureIndex(GRANT_COLLECTION, new ExpectedIndex(
                ...
                new Document("sourceType", 1)
                        .append("sourceId", 1)
                        .append("benefitCode", 1),
                true,
                null,
                null
        ));
    ...
    private void validateNoLegacyBenefitReferences() {
    ...
    private void validateNoLegacyBenefitReference(
        ...
        Document legacy = new Document("$or", List.of(
                new Document(legacyField, new Document("$exists", true)),
                new Document("benefitCode", new Document("$exists", false))
        ));
        if (mongoTemplate.getCollection(collectionName).find(legacy).limit(1).first() != null) {
            throw new IllegalStateException(
                    "Legacy benefit reference data requires an approved v3 migration."
            );
        }
    }
